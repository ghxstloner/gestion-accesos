...
export class DocumentService {
  constructor(
    ...
    @Inject(DOCUMENT_REQUIREMENT_REPOSITORY) private readonly requirements: DocumentRequirementRepositoryPort,
    @Inject(FILE_STORAGE) private readonly storage: FileStoragePort,
    @Inject(FILE_STORAGE_CONFIG) private readonly storageConfig: FileStorageConfig,
  ...
  async listForRequest(actor: AuthenticatedUser, requestId: string): Promise<RequestDocument[]> {
  ...
  async upload(actor: AuthenticatedUser, input: UploadDocumentInput): Promise<RequestDocument> {
    ...
    if (!RequestStatePolicy.isEditable(req.status)) {
      throw new BusinessRuleError(`Cannot upload files to a request in status ${req.status}`);
    }
    ...
    if (docType.kind !== 'DOCUMENT_TYPE') {
      throw new ValidationError(`${input.documentTypeId} is not a DOCUMENT_TYPE`);
    }

    // Validate subjectId references a real primary person on the request
    if (input.subjectType === 'PERSON') {
      if (!input.subjectId) {
        throw new ValidationError('subjectId is required for PERSON-subject documents');
      }
      const linked = req.personLinks.find((p) => p.personId === input.subjectId);
      if (!linked) {
        throw new BusinessRuleError(`Person ${input.subjectId} is not linked to this request`);
      }
    }
    ...
    try {
      ...
      throw err instanceof Error ? new ValidationError(`File upload failed: ${err.message}`) : err;
    }
  ...
  async review(actor: AuthenticatedUser, input: ReviewDocumentInput): Promise<DocumentReview> {
    ...
    const reviewerRoles = ['DOCUMENT_RECEIVER', 'ACCESS_DOCUMENTS_MANAGER', 'COMPANY_ADMIN', 'SYSTEM_ADMIN'];
    ...
    if (req.status !== 'UNDER_DOCUMENT_REVIEW') {
      throw new BusinessRuleError(`Documents can only be reviewed in UNDER_DOCUMENT_REVIEW status (got ${req.status})`);
    }
