...
export default function PeoplePage() {
  ...
  const filtered = useMemo(() => {
    return scopedPeople.filter((p) => {
      const fullName =
        `${p.firstName} ${p.middleName ?? ""} ${p.firstLastName} ${p.secondLastName ?? ""}`.toLowerCase();
      const matchesSearch =
        !search ||
  ...
  const columns: Column<Person>[] = [
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted">
              {identificationTypes.find((item) => item.code === r.idType)
                ?.name ?? r.idType}
              : {r.idNumber}
            </p>
          </div>
    ...
    },
    {
      ...
      cell: (r) => (
        <div>
          <p className="text-text-secondary">{r.position}</p>
          <p className="text-xs text-text-muted">{r.department}</p>
        </div>
      ),
    },
    {
      ...
      header: "Edad",
      sortable: true,
      sortValue: (r) => calcAge(r.birthDate) ?? 0,
      cell: (r) => (
        <span className="text-text-muted">
          {calcAge(r.birthDate) ?? "—"} años
        </span>
      ),
    },
    {
        ...
        rowActions={(r) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => router.push(`/people/${r.id}`)}>
                <Eye className="mr-2 h-4 w-4" />
                Ver detalle
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => router.push(`/people/${r.id}?edit=1`)}
              >
