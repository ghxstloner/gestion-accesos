...
export default function RequestsPage() {
  ...
  const { data: requestPage, isLoading } = useRequestsQuery({
    ...
    status:
      statusFilter === "ALL"
        ? undefined
        : toBackendRequestStatus(statusFilter as AccessRequest["status"]),
    companyId: companyFilter === "ALL" ? undefined : companyFilter,
    createdByUserId: role === "SOLICITANTE" ? userData?.id : undefined,
    pageSize: 200,
  });
  const requestTypes = useActiveRequestTypes();
  const requests: AccessRequest[] = (requestPage?.items ?? []).map(
    toAccessRequestSummary,
  );
  ...
  const columns: Column<AccessRequest>[] = [
    {
      ...
      cell: (r) => (
        <span className="font-medium text-text-primary">{r.number}</span>
      ),
    },
    {
      key: "type",
    ...
    },
    {
      ...
      header: "Responsable",
      cell: (r) => (
        <span className="text-text-muted">{assignedName(r.assignedTo)}</span>
      ),
    },
  ];
        ...
        {STATUS_FILTERS.map((sf) => {
          const count =
            sf.value === "ALL"
              ...
              : scopedRequests.filter((r) => r.status === sf.value).length;
          return (
            <button
              type="button"
              key={sf.value}
            ...
            >
              {sf.label}
              <span
                className={`rounded-full px-1.5 text-[10px] ${statusFilter === sf.value ? "bg-brand-200 text-brand-800" : "bg-surface-muted text-text-muted"}`}
              >
                {count}
              </span>
            </button>
          ...
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            ...
            <SelectContent>
              ...
              {requestTypes.map((type) => (
                <SelectItem
                  key={type.id}
                  value={toFrontendRequestType(type.code)}
                >
                  {type.label}
                </SelectItem>
              ))}
