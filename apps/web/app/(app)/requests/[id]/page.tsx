...
export default function RequestDetailPage() {
  ...
  const request = requestRow
    ? {
        ...
        assignedTo:
          reviewTasks.find((task) => task.status !== "COMPLETED")
  ...
  const canSubmit =
    ...
    (role === "ADMIN_EMPRESA" || role === "SOLICITANTE");
  const canEdit =
    request.status === "BORRADOR" ||
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="info">
        <TabsList className="flex h-auto flex-wrap gap-1 bg-surface-muted p-1">
          ...
          request.status === "EN_CONFECCION" ||
          request.status === "LISTA_PARA_ENTREGA" ? (
            <TabsTrigger value="issuance" className="text-xs">
              Emisión
            </TabsTrigger>
          ) : null}
        </TabsList>
            ...
            <DetailSection title="Información general">
              <dl className="space-y-3">
                ...
                {request.serviceCompany && (
                  <InfoRow
                    icon={Building2}
                    label="Empresa de asistencia"
                    value={request.serviceCompany}
                  />
                )}
                <InfoRow
                  icon={Clock}
                  ...
                  {request.tools.map((t) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-text-muted">
                          S/N: {t.serialNumber} · Cantidad: {t.quantity}
                        </p>
                      </div>
                      {t.description && (
                        <span className="text-xs text-text-muted">
              ...
              {request.accessPoints.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {request.accessPoints.map((ap) => (
                    <span
                      key={ap}
                      className="flex items-center gap-1.5 rounded-lg border border-border bg-surface-muted px-3 py-1.5 text-sm text-text-secondary"
                    >
                ...
                {request.documents.map((d) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-text-muted">
                        {d.type} · Subido el {formatDate(d.uploadedAt)}
                      </p>
                    </div>
                    {d.status === "APROBADO" && (
                      <Badge tone="success">
                ...
                {request.history.map((ev) => {
                  ...
                  return (
                    <div key={ev.id} className="relative">
                      <div
                        className={`absolute -left-[18px] top-1.5 h-3 w-3 rounded-full border-2 border-surface ${meta.tone === "success" ? "bg-success" : meta.tone === "danger" ? "bg-danger" : meta.tone === "warning" ? "bg-warning" : meta.tone === "brand" ? "bg-brand-600" : "bg-brand-400"}`}
                      />
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-text-muted">
                            {ev.actor} · {ROLES[ev.actorRole].label}
                          </p>
                          {ev.comment && (
        ...
        {(request.status === "ENTREGADA" ||
          ...
          <TabsContent value="issuance" className="mt-4">
            <DetailSection title="Información de emisión">
              <dl className="space-y-3">
                {request.issuance?.startedAt && (
                  <InfoRow
                    icon={IdCard}
                    label="Confección iniciada"
                    value={formatDateTime(request.issuance.startedAt)}
                  />
                )}
                {request.issuance?.cardNumber && (
                  <InfoRow
                    ...
                    value={request.issuance.cardNumber}
                  />
                )}
                {request.issuance?.readyAt && (
                  <InfoRow
                    icon={PackageCheck}
                    label="Lista para entrega"
                    value={formatDateTime(request.issuance.readyAt)}
                  />
                )}
                {request.issuance?.deliveredAt && (
                  <InfoRow
                    icon={CheckCircle2}
                    label="Entregada"
                    value={formatDateTime(request.issuance.deliveredAt)}
                  />
                )}
                {request.issuance?.receivedBy && (
                  <InfoRow
                    icon={User}
                    label="Recibido por"
                    value={request.issuance.receivedBy}
                  />
                )}
                {request.issuance?.deliveryObservation && (
                  <InfoRow
                    icon={FileText}
                    label="Observación"
                    value={request.issuance.deliveryObservation}
                  />
                )}
              </dl>
            </DetailSection>
