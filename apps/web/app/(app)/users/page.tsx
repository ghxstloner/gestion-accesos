...
export default function UsersPage() {
  ...
  const columns: Column<User>[] = [
    ...
    { key: 'role', header: 'Rol', sortable: true, sortValue: (r) => r.role, cell: (r) => <span className="text-text-secondary">{ROLES[r.role].label}</span> },
    { key: 'lastAccess', header: 'Último acceso', sortable: true, sortValue: (r) => r.lastAccess ?? '', cell: (r) => <span className="text-text-muted">{formatDateTime(r.lastAccess)}</span> },
    { key: 'createdAt', header: 'Creado', sortable: true, sortValue: (r) => r.createdAt, cell: (r) => <span className="text-text-muted">{formatDate(r.createdAt)}</span> },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Link href="/users/new"><Button><Plus className="mr-2 h-4 w-4" />Crear usuario</Button></Link>}
      />
