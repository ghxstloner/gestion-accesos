...
export default function ReviewsPage() {
  ...
  const requests = (requestPage?.items ?? []).map((row) => ({
    ...
    assignedTo:
      tasks.find(
        (task) => task.requestId === row.id && task.status !== "COMPLETED",
  ...
  const columns: Column<AccessRequest>[] = [
    ...
    },
    {
      ...
      header: "Prioridad",
      cell: (r) =>
        r.type === "CARNE_PERMANENTE" ? (
          <Badge tone="brand">Alta</Badge>
        ) : (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Solicitudes pendientes de revisión y aprobación"
      />
        ...
        <div className="flex flex-wrap gap-2">
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="EN_REVISION_DOCUMENTAL">
        ...
        rowActions={(r) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => router.push(`/reviews/${r.id}`)}>
