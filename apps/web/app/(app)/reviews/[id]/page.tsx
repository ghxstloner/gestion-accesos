...
export default function ReviewDetailPage() {
  ...
  const finalTask = reviewTasks.find(
    (task) => task.taskType === "FINAL_APPROVAL" && task.status !== "COMPLETED",
  );

  const isReviewer = role === "REVISOR" || role === "ADMIN_GENERAL";
  const isJefe = role === "JEFE_DOCUMENTOS" || role === "ADMIN_GENERAL";
  const canReviewDocs =
    request.status === "EN_REVISION_DOCUMENTAL" && isReviewer;
  const canApprove = request.status === "PENDIENTE_APROBACION" && isJefe;

  const docProgress =
    request.documents.length > 0
      ? Math.round(
          (request.documents.filter((d) => d.status === "APROBADO").length /
  ...
  const handleReturn = () => {
    ...
    if (!task) return;
    const reasonCode = rejectionReasons.find(
      (reason) => reason.id === returnForm.reason,
    )?.code;
    transitionReview.mutate(
      {
        taskId: task.id,
  ...
  const handleReject = () => {
    ...
    if (!task) return;
    const reasonCode = rejectionReasons.find(
      (reason) => reason.id === rejectForm.reason,
    )?.code;
    transitionReview.mutate(
      {
        taskId: task.id,
  ...
  return (
    <div className="space-y-6">
                ...
                {canReviewDocs && (
                  <>
                    <Button
                      ...
                      disabled={request.documents.some(
                        (d) =>
              ...
              <div className="h-2 w-full overflow-hidden rounded-full bg-border">
                <div
                  className={cn(
                    ...
                    docProgress === 100 ? "bg-success" : "bg-brand-600",
                  )}
              ...
              <div className="space-y-3">
                {request.documents.map((d) => (
                  ...
                  >
                    ...
                    {d.observation && (
                      <p className="mt-2 text-xs text-danger bg-danger-soft rounded p-2">
                        {d.observation}
                      </p>
                    )}
                    {canReviewDocs && (
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setReturnForm({ ...returnForm, reason: v })
                }
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setReturnForm({ ...returnForm, comment: e.target.value })
                }
                rows={4}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setRejectForm({ ...rejectForm, reason: v })
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder="Seleccione motivo" />
            ...
            <div>
              ...
              <Textarea
                ...
                rows={4}
                placeholder="Justifique el rechazo"
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog
        open={!!rejectDocId}
        onOpenChange={(o) => !o && setRejectDocId(null)}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
