...
function AdminDashboard() {
  ...
  const activity = (auditPage?.items ?? []).map((event) => ({
    ...event,
    actor:
      users.find((user) => user.id === event.actorUserId)?.firstName ??
      "Sistema",
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      returned: requests.filter((r) => r.status === "DEVUELTA_PARA_CORRECCION")
        .length,
      approved: requests.filter(
        (r) =>
          r.status === "APROBADA" ||
          r.status === "EN_CONFECCION" ||
          r.status === "LISTA_PARA_ENTREGA" ||
          r.status === "ENTREGADA",
      ).length,
    };
...
function CompanyAdminDashboard() {
  ...
  const stats = {
    ...
    returned: myReqs.filter((r) => r.status === "DEVUELTA_PARA_CORRECCION")
      .length,
    approved: myReqs.filter((r) =>
      ["APROBADA", "EN_CONFECCION", "LISTA_PARA_ENTREGA", "ENTREGADA"].includes(
        r.status,
      ),
    ).length,
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <RequestAnalytics requests={myReqs} />
      ...
      <DetailSection title="Solicitudes recientes">
        {recent.length === 0 ? (
          <EmptyState
            ...
            description="Cree su primera solicitud."
            action={
              <Link href="/requests/new">
                <Button>
                  <Plus className="mr-2 h-4 w-4" />
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Revisión documental de solicitudes"
      />
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Jefatura de documentos de acceso"
      />
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
        ...
        <StatCard
          label="En emisión"
          value={
            (credentialPage?.items ?? []).filter((credential) =>
              [
                "PENDING_PRODUCTION",
                "IN_PRODUCTION",
                "READY_FOR_DELIVERY",
...
function EmisorDashboard() {
  ...
  const credentialRequestIds = new Set(
    credentials.map((credential) => credential.requestId),
  );
  const pending = requests.filter(
    (request) =>
      request.status === "APROBADA" && !credentialRequestIds.has(request.id),
  );
  const inProgress = credentials.filter(
    (credential) =>
      credential.status === "PENDING_PRODUCTION" ||
      credential.status === "IN_PRODUCTION",
  );
  const ready = credentials.filter(
    (credential) => credential.status === "READY_FOR_DELIVERY",
  );
  const delivered = credentials.filter(
    (credential) => credential.status === "DELIVERED",
  );

  return (
    <div className="space-y-6">
      <PageHeader
        title="Emisión de carnés"
        description="Confección y entrega de carnés"
      />
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          label="Pendientes de confección"
          value={pending.length}
          icon={Clock}
          tone="warning"
        />
